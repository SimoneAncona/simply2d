...
export function setRectangle(renderer, x, y, width, height, r, g, b, a, fill) {
    sdl2bind.setDrawColor(renderer, r, g, b, a);
    sdl2bind.drawRectangle(renderer, x, y, width, height, fill);
    if (!renderingSequence)
        sdl2bind.renderPresent(renderer);
}
export function renderPresent(renderer) {
    sdl2bind.renderPresent(renderer);
}
...
export function watchRawData(renderer, width, height) {
    return new Uint8Array(sdl2bind.readData(renderer, width, height));
}
export function savePNG(renderer, width, height, filename) {
    sdl2bind.savePNG(renderer, width, height, filename);
}
export function saveJPG(renderer, width, height, filename) {
    sdl2bind.saveJPG(renderer, width, height, filename);
}
export function setRenderScale(renderer, width, height, scale) {
    sdl2bind.setScale(renderer, width, height, scale);
}
export function onClickEvent(callback) {
    sdl2bind.onClick(callback);
}
export function onKeyDownEvent(callback) {
    sdl2bind.onKeyDown(callback);
}
export function onKeyUpEvent(callback) {
    sdl2bind.onKeyUp(callback);
}
export function onKeysDownEvent(callback) {
    sdl2bind.onKeysDown(callback);
}
export function onKeysUpEvent(callback) {
